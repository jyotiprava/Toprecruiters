...
$uniquekey=md5($firstname.'--'.$lastname.'--'.$email.'--'.$join_date.'--'.time());
	$sql=mysql_query("select * from `user_detail` where `emailid`='$email'")or die(mysql_error());
    $num=mysql_num_rows($sql);
    if($num==0)
    {
    mysql_query("insert into `user_detail` set `first_name`='$firstname',`last_name`='$lastname',`emailid`='$email',`password`='$password',`join_date`='$join_date',`uniquekey`='$uniquekey'")or die(mysql_error());
