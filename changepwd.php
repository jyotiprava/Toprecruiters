...
if(isset($_POST['Submit']))
{
	...
	$qwery=mysql_query("select * from `user_detail` where `emailid`='$_SESSION[userid]' and `password`='$oldpwd' and `is_employer`='0'")or die(mysql_error());
	if(mysql_num_rows($qwery)>0)
	{
	$newpwd=md5(htmlentities($_POST['newpwd'],ENT_QUOTES));
	$que=mysql_query("UPDATE `user_detail` SET `password`='$newpwd' WHERE `emailid`='$_SESSION[userid]' and `is_employer`='0'");
    	if($que)
	{
...
<tr>
<td>Emailid</td>
<td><input type="text" name="username" value="<?php echo $_SESSION['userid'];?>" class="textbox"/></td>
</tr>
 <tr>
    <td>Old Password </td> 
    <td><input type="password" name="oldpwd" class="textbox" id="oldpwd" required onblur="return valid($('#oldpwd').val());"/></td>
  </tr>   
  <tr>
    <td>New Password </td>
