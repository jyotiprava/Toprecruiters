...
<script type="text/javascript">
		function delete_data(vals){
			...
			if(con){
			window.location="employee_delete.php?id="+vals;
			}
		}
								...
								<div id="content2" style="min-height:350px;">
									...
									?>   
                                             <table class="table1"  >
                                                ...
                                                <?php 
											$sqluser=mysql_query("select * from `user_detail` where `is_employer`='0'")or die(mysql_error());
											while($resuser=mysql_fetch_array($sqluser)){
											$sqlcv=mysql_query("select * from   `cv_detail` where `user_id`='$resuser[slno];'")or die(mysql_error());
											$cvres=mysql_fetch_array($sqlcv);
											//echo $resuser['emailid']."<br/>".$resuser['slno'];				
											?>
                                            <tr>
												<td><?php echo $resuser['first_name'].$resuser['last_name'];?></td>
												<td><?php echo $resuser['emailid'];?></td>
												<td><?php echo $resuser['join_date'];?></td>
												<td><a href="../pdf_server.php?file=<?php echo $cvres['cv']; ?>"><?php echo $resuser['first_name'].".cv"?></a></td>
												<td>
